...
public class CTRSwerveModuleIO implements SwerveModuleIO {
    ...
    public void updateInputs(SwerveModuleIOInputs inputs) {
        ...
        inputs.swerveModuleState = m_internalState;
        //TODO: See if this value is continuous!
        inputs.drivePositionRad = Rotation2d.fromRotations(drive_rot).getRadians();
        inputs.driveVelocityRadPerSec = m_driveVelocity.getValue();
        //TODO: Mechanical advantage used a double array in their code, find out if we need to do that.
        inputs.driveTempCelcius = m_driveMotor.getDeviceTemp().getValue();
        inputs.turnPositionRad = Rotation2d.fromRotations(angle_rot).getRadians();
        inputs.turnVelocityRadPerSec = m_steerVelocity.getValue();
        inputs.turnTempCelcius = m_steerMotor.getDeviceTemp().getValue();
    }
    @Override
